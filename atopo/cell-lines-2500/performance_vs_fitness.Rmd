...
The next tables and box-plots will help us determine exactly which cell line and dataset
to use:
...
We also compare the MCC values and fitness scores across all cell lines between
...
ggboxplot(data, x = "cell.line", y = c("MCC cell-specific", "MCC random"),
          ...
          xlab = "Cell Lines", ylab = "MCC values", add = "point")
ggboxplot(data, x = "cell.line", y = c("fitness cell-specific", "fitness random"),
          palette = brewer.pal(3, "Set1"), merge = "asis",
          xlab = "Cell Lines", ylab = "Fitness values", add = "point")
```
...
- **The cell-specific models have always a larger maximum fitness value and in
general larger overall fitness values** than the random ones for each respective cell line.
