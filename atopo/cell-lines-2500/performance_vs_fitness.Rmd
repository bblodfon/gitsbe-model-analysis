...
We will sample the data (stabilizing the seed number for reproducibility purposes)
by taking a random part of specific size of the unique models - those that have 
strictly different boolean equations:
```{r}
# For reproducibility
set.seed(0)
sample.size = 1000
...
First, we check if the our data is normally distributed (using the Shapiro-Wilk
...
From the correlation plots above, we observe a mild positive correlation between
performance and fitness to the steady state.

### Correlation of fitness between different TP-classes

```{r TP-classification of fitness values}
# Box plots
ggboxplot(df, x = "tp.unique", y = "fit.unique", color = usefun:::colors.100[1:length(unique(tp.unique))],
          xlab = "True Positives (TP)", ylab = "Fitness values")
...
ht = Heatmap(matrix = p.value.mat, cluster_rows = FALSE, cluster_columns = FALSE,
  ...
  cell_fun = function(j, i, x, y, width, height, fill) {
    if (!is.na(p.value.mat[i,j]))
        grid.text(sprintf("%.6f", p.value.mat[i, j]), x, y, gp = gpar(fontsize = 10))
})
draw(ht)
...
### Correlation of fitness between different MCC-classes
