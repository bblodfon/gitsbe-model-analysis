...
We will demonstrate that these inhibited biomarkers are a direct consequence of the overepression of `ERK_f`.
...
for (i in 1:nrow(AK.PD.res)) {
  ...
  #plot_avg_state_diff_graph(net, diff = diff.state.ak.pd, layout = nice.layout, title = paste0("Set: ", synergy.set, "(", synergy.set.models.num, ") vs Subset: ", synergy.subset, "(", synergy.subset.models.num, ")"))
  #plot_avg_link_operator_diff_graph(net, diff = diff.link.ak.pd, layout = nice.layout, title = paste0("Set: ", synergy.set, "(", synergy.set.models.num, ") vs Subset: ", synergy.subset, "(", synergy.subset.models.num, ")"))
}
...
threshold = 0.8
biomarker.state.mat = apply(diff.state.mat, c(1,2), function(x) {
...
```{r AK-PD,BI-PD,PD-PI vs BI-PD,PD-PI ctivity state graph, cache=TRUE}
...
plot_avg_state_diff_graph(net, diff = diff.state.mat["AK-PD,BI-PD,PD-PI vs BI-PD,PD-PI", ], layout = nice.layout, 
...
threshold = 0.8
biomarker.link.mat = apply(diff.link.mat, c(1,2), function(x) {
  if (x >= threshold | x <= -threshold) 1 else 0
})
...
The above means that there were $44$ nodes that were *zero* times found as link operator biomarkers across all synergy set comparisons, one that was found once, one that was found twice, **$5$ that were found 6 times and $1$ that was found 11 times**. The last node is of interest to us and it is:
...
The boolean equation of the `SRC` node is: `SRC *=  (  RTPK_f ) AND/OR NOT  ( CSK )` and it's mean link operator difference value across all synergy set comparisons is `r mean(diff.link.mat[, "SRC"])` which means that on average the logic operator that binds its two regulators is the `AND NOT` and thus it depends on both of it's regulators equally (meaning that it's more difficult to have it as activated - 1 case out of 4). This correlates with the fact that it was found as mostly inhibited in the analysis above.

For example, visualizing the link operator difference of the synergy sets `AK-PD,BI-PD,PD-PI` vs `BI-PD,PD-PI` from Table 2:

```{r AK-PD,BI-PD,PD-PI vs BI-PD,PD-PI link operator diff graph, cache=TRUE}
plot_avg_link_operator_diff_graph(net, diff = diff.link.mat["AK-PD,BI-PD,PD-PI vs BI-PD,PD-PI", ], layout = nice.layout, 
  title = paste0("Set: ", synergy.set, " (", synergy.set.models.num, " models) vs Subset: ", synergy.subset, " (", synergy.subset.models.num, " models)"))
```

<div class="blue-box">
We conclude that cancer models in which the `AK-PD` drug combination is synergistic, tend to have the nodes `SRC` and `PTPN6` in an **inhibited state** and the `SRC` node depends on both it's regulators, `RTPK_f` and `CSK`.
</div>
</br>
...
We first construct two matrices: in the first, **each row is a set vs subset average activity difference vector of the network nodes**, while on the second **each row is a set vs subset average link operator difference vector of the network nodes**:
```{r Synergy Subsets Comparison (Random - A498), cache=TRUE}
...
caption.title.1 = "Table 3: Average activity difference values across all synergy set comparisons (AK-PD)"
...
caption.title.2 = "Table 4: Average link operator difference values across all synergy set comparisons (AK-PD)"
datatable(data = diff.link.mat[, c("SRC", "RAC_f", "MEK_f", "STAT1", "PTEN")], options = list(
    searching = FALSE, pageLength = 5, lengthMenu = c(5, 10)),
  caption = htmltools::tags$caption(caption.title.2, style="color:#dd4814; font-size: 18px")) %>% 
  formatRound(1:ncol(diff.link.mat), digits = 3)
...
threshold = 0.7
biomarker.state.mat.random = apply(diff.state.mat.random, c(1,2), function(x) {
  if (x >= threshold | x <= -threshold) 1 else 0
})
...
But the total number of comparisons was `r nrow(biomarker.state.mat.random)` so we could argue that **this is not a statistically significant result**, though the `PDPK1` node can also be seen as an inhibited biomarker at a lower singnificant threshold level from the figure above with the activity state biomarkers from the random models.
...
threshold = 0.7
biomarker.link.mat.random = apply(diff.link.mat.random, c(1,2), function(x) {
  if (x >= threshold | x <= -threshold) 1 else 0
})
...
So, there was a node that was found $7$ times (out of a total of `r nrow(biomarker.link.mat.random)`) as a link operator biomarkers across all synergy set comparisons (so also **not statistically significant**):
