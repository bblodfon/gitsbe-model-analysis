...
The `G2` drug is a **PDPK1 inhibitor** and the `D1` drug is **an inhibitor of `RSK` isoforms** (
...
comtions that were found synergistic across all the 
...
```{r Average activity state diff across all synergy subsets (AK-PD), cache=TRUE}
...
```{r Average activity state diff across all synergy subsets (AK-PD) - dotchart, cache=TRUE}
...
We will now check if the above observations regarding the node activity are true compared to the [MCLP dataset](https://tcpaportal.org/mclp/#/download):
...
We check the expression value of the `SRC` across all cell lines:
```{r MCLP Data Visualization (SRC), cache=TRUE}
src.data = mclp.data %>% select(Sample_Name, SRC) %>% na.omit()
...
ggdotchart(data = src.data, x = "Sample_Name", y = "SRC", 
  title = "SRC expression across all cell lines in MCLP Dataset",
  ...
  xlab = "Cell Lines", ylab = "SRC exression",
  add.params = list(color = "activity", palette = c("red", "grey", "green"))) + 
    theme(axis.text.x = element_blank(), axis.ticks = element_blank())
```

For the `ERK2` we don't have data for the `A498` cell line:
```{r MCLP Data Visualization (ERK2), cache=TRUE}
erk.data = mclp.data %>% select(Sample_Name, ERK2) %>% na.omit()

# find the activity classes (3 classes: low expression, no expression, high expression)
res = Ckmeans.1d.dp(x = erk.data$ERK2, k = 3)
activity = as.factor(res$cluster)
levels(activity) = c("low", "medium", "high")
erk.data = cbind(erk.data, activity)

ggdotchart(data = erk.data, x = "Sample_Name", y = "ERK2", 
  title = "ERK2 expression across all cell lines in MCLP Dataset",
  color = "activity", palette = c("red", "grey", "green"), 
  label = "Sample_Name", label.select = cell.lines.in.mclp, repel = TRUE, xlab = "Cell Lines",
  add = "segments", label.rectangle = TRUE, ylab = "ERK2 expression",
  add.params = list(color = "activity", palette = c("red", "grey", "green"))) + 
    theme(axis.text.x = element_blank(), axis.ticks = element_blank())
```
...
```{r Average link operator diff across all synergy subsets (AK-PD), cache=TRUE}
...
But the total number of comparisons was `r nrow(biomarker.state.mat.random)` so we could argue that **this is not a statistically significant result**, which can be seen clearly in the next graph where we visualize the **average activity state difference across all synergy comparison sets** from Table 3:

```{r Average activity state diff across all synergy subsets (AK-PD, random models), cache=TRUE}
...
```{r Average link operator diff across all synergy subsets (AK-PD, random), cache=TRUE}
