...
- **PD-PI** (A498 cell line)
  ...
  - `PTEN` inhibition - in [cell-specific models](#Text1) (with `ERK_f` overexpression)
  - `SOS1` [inhibition](#sos1) in all cell lines except `A498` and `SW620`
- **PD-G2** (A498 cell line)
  - `ERK_f` overexpression ([random models](#Text5))
  - `PDPK1` [overexpression](#Text6)
  - `PTEN` inhibition (see [here](#Text3) and [here](#Text4))
  - `PIK3CA` and `PTEN` share an [`OR NOT` relationship as regulators of `PDPK1`](#pdpk1-eq)
  - `SOS1` [inhibition](#sos2) in all cell lines except `A498` and `SW620`
- **PI-D1** (all 9 model datasets)
  - `ERK_f` [overexpression](#Table9)
  - `CDC42` [inhibition](#Table9)
</div>
...
In this section, we will **visualize the cell lines' observed synergies and mark the synergies that were also predicted by the cell-specific models and the random-generated ones (in one cell line at least)**. 
...
Thus, we can still see if there are any common (activity state and link operator) biomarkers for `AK-PD` across the all the cell-specific models by contrasting in each cell line the models that predicted `AK-PD` (if there were any) vs the models that did not:
...
Though the `PD-PI` synergy was observed and predicted in the **A498 model dataset**, we investigate its biomarkers in the other cell lines where **it was predicted as a False Positive (FP) synergy (predicted by the models but not observed in the experiments)**. 
Thus, we can still see if there are any common (activity state and link operator) biomarkers for this synergy across the all the cell-specific models by contrasting in each cell line the models that predicted `PD-PI` (if there were any) vs the models that did not:

```{r PD-PI average activity state difference in all cell lines}
drug.comb = "PD-PI"
...
Now, we **identify the biomarkers of interest** as those nodes that have surpassed a user-defined threshold ($0.6$) across as many as possible cell lines - counting thus the *frequency* that this has occured. 
We show those nodes that surpassed the threshold in at least half of the cell lines:
```{r PD-PI activity biomarkers frequency across cell lines, results = 'asis'}
...
So we identified two biomarkers, `ERK_f` (known from before) and `SOS1`.
The average activity difference values for the `SOS1` per cell line model datasets were:
<div id="sos1"></div>
```{r SOS1 average diff values (PD-PI)}
...
- [DU145](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3727633/)
- [AGS and other gastric cell lines](https://doi.org/10.1038/s41598-018-24969-w)
- [SW620](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5921181/)
...
```{r MCLP Data Visualization (PDPK1), cache=TRUE, fig.align='center'}
...
```{r MCLP Data Visualization (PIK3CA), cache=TRUE, fig.align='center'}
...
<div class="orange-box" id="pdpk1-eq">
So, the results above hint that `PIK3CA` might be inhibited in the `A498` cell line, which considering the `PTEN` **inhibition**, `PDPK1` **overexpression** and their boolean equation (which connects all of them together): `PDPK1 *= PIK3CA and/or not PTEN`, we conclude that there must be an `or not` link operator between the 2 regulators.
</div>
<br>
<div class="blue-box">
...
Also, `PIK3CA` has an `or not` (link operator) relationship with `PTEN` (according to our model's equation) as regulators of `PDPK1`.
</div>
</br>

### PD-G2 in other cell lines

Though the `PD-G2` synergy was observed and predicted in the **A498 model dataset**, we investigate its biomarkers in the other cell lines where **it was predicted as a False Positive (FP) synergy (predicted by the models but not observed in the experiments)**.
Thus, we can still see if there are any common (activity state and link operator) biomarkers for this synergy across the all the cell-specific models by contrasting in each cell line the models that predicted `PD-G2` (if there were any) vs the models that did not:

```{r PD-G2 average activity state difference in all cell lines}
drug.comb = "PD-G2"
...
Now, we **identify the biomarkers of interest** as those nodes that have surpassed a user-defined threshold ($0.6$) across as many as possible cell lines - counting thus the *frequency* that this has occured. 
We show those nodes that surpassed the threshold in at least half of the cell lines:
```{r PD-G2 activity biomarkers frequency across cell lines, results = 'asis'}
biomarker.state.mat = binarize_to_thres(mat = diff.state.mat, thres = 0.6)
biomarkers.freq = colSums(biomarker.state.mat)/nrow(biomarker.state.mat)
...
The average activity difference values for the `SOS1` per cell line model datasets were:
<div id="sos2"></div>
```{r SOS1 average diff values (PD-G2)}
datatable(data = as.data.frame(diff.state.mat[,'SOS1']), colnames = "SOS1", 
  options = list(dom = 't', order = list(list(1, 'asc'))), width = "470px",
...
I found some [articles](#sos1) that I believe show that `SOS1` is expressed in some of the above cancer cell lines (in contrast to the results [above](#sos2)).

## PI-D1 activity biomarkers {-}

As observed in the [heatmap above](#observed-synergies), the `PI-D1` synergy was predicted by both the **cell specific** and **random models** in **some of the cell lines tested**.
So, for each seperate model dataset, we are going to contrast the models that predicted the `PI-D1` synergy vs the models that did not (if these two sets of models do exist for each cell line) and find the average activity difference per node in each case:

```{r PI-D1 average activity state difference in all cell lines}
drug.comb = "PI-D1"
...
Now, we **identify the biomarkers of interest** as those nodes that have surpassed a user-defined threshold ($0.6$) across as many as possible cell lines - counting thus the *frequency* that this has occured. 
We show those nodes that surpassed the threshold in at least half of the cell lines:
```{r PI-D1 activity biomarkers frequency across cell lines, results = 'asis'}
biomarker.state.mat = binarize_to_thres(mat = diff.state.mat, thres = 0.6)
biomarkers.freq = colSums(biomarker.state.mat)/nrow(biomarker.state.mat)
...
So, `ERK_f` was a biomarker for all the cell lines that predicted the `PI-D1` synergy and the nodes `ILK`, `CDC42` and `PAK1` in more than half of them.
If we see the actual numbers we can clarify their state as well:

<div id="Table9"></div>
```{r PI-D1 biomarkers (table 9)}
...
<div class="orange-box">
If we examine the boolean equations: `ILK *= PAK1`, `PAK1 *= CDC42 or RAC_f` and `CDC42 *= SRC and/or not ARHGAP24` as well as the information from the [table 9](#Table9), then we can clearly see that **the only inhibited node of interest is `CDC42` among the 3 most frequent inhibited biomarkers**.
</div>
<br>
