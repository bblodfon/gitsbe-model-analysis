...
```{r Normality testing, warning=FALSE, comment=""}
...
<div class="green-box">
So, we see that the new steady state vector for each cell line is around half different than the previous calculated one.
</div>
</br>
...
So, it seems that the 2 computational methods are very different even when compared on the node subset that constitutes the gold standard. Both of them are half correct when it comes to true (curated) steady state predictions.
</div>
</b>
...
Then, we check if the our data is normally distributed (using the Shapiro-Wilk
test for normality and the Q-Q plots):
```{r Normality testing 2, warning=FALSE, comment=""}
ggqqplot(data = df, x = "fit.unique", ylab = "Fitness")
ggqqplot(data = df, x = "mcc.unique", ylab = "MCC")
...
```{r Rank correlation plots: Fitness to curated AGS ss vs MCC}
